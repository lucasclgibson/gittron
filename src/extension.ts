...
export function activate(context: vscode.ExtensionContext) {
	...
	const hoverProvider = vscode.languages.registerHoverProvider('*', {
		provideHover(document, position, token) {
			...
			if (lineNumber === targetLine) {
				...
				markdown.appendMarkdown(`**Comment by [@${activeComment.user.login}](${activeComment.html_url})**\n\n`);
	...
	async function refreshComments(includeResolved: boolean = false, forceNewPR: boolean = false): Promise<void> {
		if (!currentPRInfo || forceNewPR) {
			...
			try {
				...
				await vscode.window.withProgress({
					...
					progress.report({ message: `Detected repository: ${repoInfo.owner}/${repoInfo.name}` });
					...
					try {
						...
						const prInput = await vscode.window.showInputBox({
							...
							placeHolder: 'e.g., 123',
							validateInput: (input) => {
								return /^\d+$/.test(input) ? null : 'Please enter a valid PR number (digits only)';
							}
						});
					...
					progress.report({ message: `Getting ${includeResolved ? 'all' : 'unresolved'} comments for PR #${prNumber}` });
				...
				vscode.window.showInformationMessage('PR comments fetched successfully.');
			} catch (error) {
				if (error instanceof Error) {
					vscode.window.showErrorMessage(`Error fetching PR comments: ${error.message}`);
				} else {
			...
			try {
				await vscode.window.withProgress({
					...
					progress.report({ message: `Getting ${includeResolved ? 'all' : 'unresolved'} comments for PR #${currentPRInfo!.number}` });
					...
					// Set PR info in the comments provider
					if (currentPRInfo) {
						commentsProvider.setPRInfo(currentPRInfo.owner, currentPRInfo.repo, currentPRInfo.number);
					}
					
					return comments;
				});
				vscode.window.showInformationMessage('PR comments refreshed successfully.');
			} catch (error) {
				if (error instanceof Error) {
					vscode.window.showErrorMessage(`Error refreshing PR comments: ${error.message}`);
				} else {
					vscode.window.showErrorMessage('Unknown error refreshing PR comments');
				}
			}
		}
	}
	...
	const setGitHubTokenCommand = vscode.commands.registerCommand('gittron.setGitHubToken', async () => {
	...
	const commands = [
		...
		vscode.commands.registerCommand('gittron.copyAsAgentInstruction', async () => {
			console.log('Copy as Agent Instruction command executed');
			
			if (!activeComment || !activeCommentLine) {
				vscode.window.showWarningMessage('No active comment or code line to copy');
				return;
			}
...
`;
				...
				vscode.window.showInformationMessage('Agent instruction copied to clipboard');
			} catch (error) {
		...
		vscode.commands.registerCommand('gittron.toggleResolvedComments', async () => {
			console.log('Toggle Resolved Comments command executed');
			
			if (!currentPRInfo) {
				vscode.window.showWarningMessage('No PR information available. Please fetch PR comments first.');
				return;
			}
			...
			// Toggle the setting
			await config.update('includeResolvedComments', !includeResolved, vscode.ConfigurationTarget.Global);
		...
		vscode.commands.registerCommand('gittron.handleComment', async (comment: PullRequestComment) => {
			console.log('Handle Comment command executed with data:', JSON.stringify({
				id: comment.id,
			...
			if (comment.path && (comment.line || comment.position)) {
				console.log(`Opening file comment at ${comment.path}:${comment.line || comment.position}`);
				...
				try {
					...
					try {
						...
						console.log('Could not find file at direct path, trying to search for filename...');
						...
						if (fileName) {
							...
							if (files.length > 0) {
								console.log('Found file matches:', files.map(f => f.toString()).join(', '));
								fileUri = files[0];
								document = await vscode.workspace.openTextDocument(fileUri);
							} else {
								throw new Error(`File ${fileName} not found in the workspace`);
							}
						}
					}
					...
					if (targetLine !== lineNumber) {
						console.log(`Adjusted line number from ${lineNumber} to ${targetLine} (document has ${lineCount} lines)`);
					}
					...
					const range = new vscode.Range(
						...
						new vscode.Position(targetLine, document.lineAt(targetLine).text.length)
					);
	...
	vscode.commands.getCommands(true).then(commands => {
		...
		commands.filter(cmd => cmd.startsWith('gittron.')).forEach(cmd => {
			console.log(`- ${cmd}`);
		});
	});
}
